...
export default memo(function RoutineCard({routine,onPause,onDelete,onComplete,onTalk,delay=0}){
  ...
  return(<FadeIn delay={delay}><div style={{padding:"16px 18px",borderRadius:18,marginBottom:10,background:routine.paused?"rgba(0,0,0,0.02)":justCompleted?(seg?.soft||C.accSoft):C.card,boxShadow:C.shadow,position:"relative",borderLeft:"4px solid "+(routine.paused?C.t3:seg?.color||C.acc),opacity:routine.paused?.5:1,transition:"background 0.3s"}}>
    ...
    <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
      ...
      <button onClick={function(){onTalk('Generate a fresh step for my "'+routine.title+'" habit. Search for something specific and new.');}} style={{...F,fontSize:11,padding:"6px 12px",borderRadius:10,background:C.accSoft,border:"1px solid "+C.accBorder,color:C.acc,cursor:"pointer",fontWeight:600}}>Generate now</button>
      <button onClick={function(){onTalk('Update my "'+routine.title+'" habit. Make it better or change the schedule.');}} style={{...F,fontSize:11,padding:"6px 12px",borderRadius:10,background:C.cream,border:"none",color:C.t2,cursor:"pointer"}}><Sparkles size={14}/> Improve</button>
    </div>
  </div></FadeIn>);
})
